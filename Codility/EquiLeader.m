...
+ (int)solution:(NSMutableArray *)A {
    ...
    for (int i = 1; i < count; i++) {
        ...
        else if (leaderValue == currentValue) {
            leaderCount++;
        }
        else {
            leaderCount--;
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (currentValue == leaderValue) {
            totalLeaders++;
        }
    }
    
    // no leader
    if (totalLeaders <= count / 2) {
        return 0;
    }
    
    int sinisterLeaders = 0;
    int result = 0;
    ...
    for (int i = 0; i < count; i++) {
        ...
        int dexterLeaders = totalLeaders - sinisterLeaders;
        
        if (sinisterLeaders > (i + 1) / 2 && dexterLeaders > (count - i - 1) / 2) {
            result++;
        }
    }
